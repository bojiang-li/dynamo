...
For Dynamo request traces and Weka corpora, omit `traffic.source.block_size`; AISimulate derives it
from the source and rejects mixed block sizes. An explicitly configured Weka block size is only an
assertion against the published source metadata. Dynamo format accepts multiple trace shards. Weka
accepts one file or directory; other formats accept exactly one file.
...
[AISimulate](https://github.com/ai-dynamo/aisimulate) owns public Weka ingestion, validation, and
lowering into the canonical `ValidatedAgenticGraph`. The lower-level
...
Planner, and Mocker runtime behavior.
